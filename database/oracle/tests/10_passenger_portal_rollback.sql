...
BEGIN
    ...
    v_before := smartmove_owner.available_seats(2);
    smartmove_owner.create_booking(990001, 1, 2);
    smartmove_owner.reserve_seat(990001, 990001, 4);
    smartmove_owner.reserve_seat(990002, 990001, 5);
    ensure(smartmove_owner.available_seats(2) = v_before - 2, 'Two seats were not held');
    smartmove_owner.web_cancel_booking(990001, 1, v_changed, v_refund);
    ensure(v_changed = 1 AND v_refund = 0, 'Pending cancellation result');
    ensure(smartmove_owner.available_seats(2) = v_before, 'Pending seats not released');
    ...
    smartmove_owner.web_cancel_booking(2, 2, v_changed, v_refund);
    ...
    SELECT COUNT(*) INTO v_count FROM smartmove_owner.refunds WHERE payment_id = 2;
    ensure(v_count = 1, 'Duplicate refund created');
    DBMS_OUTPUT.PUT_LINE('PASS paid multi-seat cancellation, full refund and repeat');

    BEGIN
        smartmove_owner.web_cancel_booking(1, 1, v_changed, v_refund);
    ...
    BEGIN
        smartmove_owner.web_cancel_booking(2, 1, v_changed, v_refund);
    ...
    DELETE FROM smartmove_owner.feedback_records WHERE feedback_id = 1;
    smartmove_owner.web_submit_feedback(990001, 1, 1, 'REVIEW');
    BEGIN
        smartmove_owner.web_submit_feedback(990002, 1, 1, 'REVIEW');
        RAISE_APPLICATION_ERROR(-20990, 'Duplicate review accepted');
    EXCEPTION WHEN DUP_VAL_ON_INDEX THEN NULL;
    END;
    BEGIN
        smartmove_owner.web_submit_feedback(990003, 2, 2, 'REVIEW');
        RAISE_APPLICATION_ERROR(-20990, 'Uncompleted trip review accepted');
    EXCEPTION WHEN OTHERS THEN
        IF SQLCODE != -20024 THEN RAISE; END IF;
    END;
    smartmove_owner.web_submit_feedback(990004, 3, 1, 'COMPLAINT');
    BEGIN
        smartmove_owner.web_submit_feedback(990005, 3, 2, 'COMPLAINT');
    ...
    smartmove_owner.web_update_profile(1, 'Rollback Profile', '0771234567');
    SELECT full_name INTO v_status FROM smartmove_owner.passengers WHERE passenger_id = 1;
