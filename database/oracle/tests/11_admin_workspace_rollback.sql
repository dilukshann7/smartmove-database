...
BEGIN
  ...
  SELECT COUNT(*) INTO v_count FROM smartmove_owner.web_admin_login WHERE user_id=990100;
  ensure(v_count=1,'Admin login view failed');
  SELECT COUNT(*) INTO v_count FROM smartmove_owner.web_admin_login WHERE user_id=1;
  ensure(v_count=0,'Passenger entered admin login view');
  SELECT COUNT(*) INTO v_count FROM smartmove_owner.web_passenger_login WHERE user_id=990100;
  ensure(v_count=0,'Admin entered passenger login view');
  DBMS_OUTPUT.PUT_LINE('PASS admin role and session');

  smartmove_owner.add_route(990101,'Rollback Route','Test A','Test B',400);
  smartmove_owner.update_route(990101,'Updated Route',450);
  smartmove_owner.add_vehicle(990101,'TEST-ADMIN-11','VAN',10);
  smartmove_owner.add_driver(990101,'Rollback Driver','0711111111','TEST-LIC-11');
  smartmove_owner.schedule_trip(990101,990101,990101,990101,SYSDATE+3,SYSDATE+3+2/24);
  BEGIN
    smartmove_owner.schedule_maintenance(990101,990101,'Conflict check',SYSDATE+3);
  ...
  smartmove_owner.reschedule_trip(990101,990101,990101,SYSDATE+4,SYSDATE+4+2/24);
  smartmove_owner.delete_trip(990101);
  smartmove_owner.schedule_maintenance(990101,990101,'Routine',SYSDATE+4);
  smartmove_owner.delete_maintenance(990101);
  smartmove_owner.delete_driver(990101);
  smartmove_owner.delete_vehicle(990101);
  smartmove_owner.delete_route(990101);
  DBMS_OUTPUT.PUT_LINE('PASS guarded route, trip, fleet and maintenance edits');

  smartmove_owner.create_booking(990100,1,2);
  smartmove_owner.reserve_seat(990100,990100,4);
  smartmove_owner.web_admin_record_payment(990100,'CASH',v_payment,v_amount);
  ensure(v_amount=800,'Payment must be full fare');
  SELECT COUNT(*) INTO v_count FROM smartmove_owner.tickets
    WHERE booking_id=990100 AND status='ISSUED';
  ensure(v_count=1,'Ticket was not issued');
  smartmove_owner.cancel_booking(990100);
  smartmove_owner.web_admin_record_refund(990100,v_changed,v_amount);
  ...
  SELECT COUNT(*) INTO v_count FROM smartmove_owner.refunds WHERE payment_id=v_payment;
  ensure(v_count=1,'Duplicate refund record');
  DBMS_OUTPUT.PUT_LINE('PASS payment, ticket issue and one full refund');

  smartmove_owner.web_admin_cancel_trip(2,v_changed,v_refunds);
  ...
  SELECT COUNT(*) INTO v_count FROM smartmove_owner.refunds
