...
CREATE SEQUENCE smartmove_owner.web_refund_seq START WITH 100000 INCREMENT BY 1 NOCACHE;
CREATE SEQUENCE smartmove_owner.web_feedback_seq START WITH 100000 INCREMENT BY 1 NOCACHE;

CREATE UNIQUE INDEX smartmove_owner.web_one_review_per_booking
    ON smartmove_owner.feedback_records
    (CASE WHEN feedback_type = 'REVIEW' THEN booking_id END);

CREATE TABLE smartmove_owner.web_notification_baseline (
    baseline_id NUMBER PRIMARY KEY CHECK (baseline_id = 1),
    started_at DATE NOT NULL
);
INSERT INTO smartmove_owner.web_notification_baseline VALUES (1, SYSDATE);
...
CREATE OR REPLACE PROCEDURE smartmove_owner.web_cancel_booking (
    p_booking_id IN NUMBER, p_passenger_id IN NUMBER,
    p_changed OUT NUMBER, p_refund_amount OUT NUMBER
) AS
    v_status smartmove_owner.bookings.status%TYPE;
    v_departure DATE;
    v_payment_id NUMBER;
...
BEGIN
    ...
    smartmove_owner.cancel_booking(p_booking_id);
    IF v_status = 'CONFIRMED' THEN
        v_refund_id := smartmove_owner.web_refund_seq.NEXTVAL;
        smartmove_owner.process_refund(v_refund_id, v_payment_id,
...
CREATE OR REPLACE PROCEDURE smartmove_owner.web_update_profile (
    p_passenger_id IN NUMBER, p_name IN VARCHAR2, p_phone IN VARCHAR2
) AS
BEGIN
    UPDATE smartmove_owner.passengers
...
CREATE OR REPLACE PROCEDURE smartmove_owner.web_submit_feedback (
    p_feedback_id IN NUMBER, p_booking_id IN NUMBER,
    p_passenger_id IN NUMBER, p_type IN VARCHAR2
) AS
    v_booking_status smartmove_owner.bookings.status%TYPE;
    v_trip_status smartmove_owner.trips.status%TYPE;
BEGIN
    ...
    END IF;
    INSERT INTO smartmove_owner.feedback_records
...
GRANT SELECT ON smartmove_owner.payments TO smartmove_web;
GRANT SELECT ON smartmove_owner.refunds TO smartmove_web;
GRANT SELECT ON smartmove_owner.feedback_records TO smartmove_web;
GRANT SELECT ON smartmove_owner.booking_status_history TO smartmove_web;
GRANT SELECT ON smartmove_owner.web_notification_baseline TO smartmove_web;
GRANT SELECT ON smartmove_owner.web_feedback_seq TO smartmove_web;
GRANT EXECUTE ON smartmove_owner.web_cancel_booking TO smartmove_web;
GRANT EXECUTE ON smartmove_owner.web_update_profile TO smartmove_web;
GRANT EXECUTE ON smartmove_owner.web_submit_feedback TO smartmove_web;
